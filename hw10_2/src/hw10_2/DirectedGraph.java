...
public class DirectedGraph {
	private Node[] list;
	private Node r; // 위상 정렬을 저장할 노드 r
	private int n;
	...
	public void DFS_TS(boolean[] visited, int v) {
		visited[v] = true; // 현재 v는 방문하게 된것이므로 true로 만들어줌
		Node temp = list[v]; // temp에 정점 v의 리스트 저장
		
		while(temp != null) { // temp가 null이 아니면 계속 반복
			int x = temp.vertex; // x에 temp의 정점값을 저장해줌
			if(visited[x] == false)
				DFS_TS(visited, x); // 만약 x가 아직 방문하지 않은 정점이면 재귀
			temp = temp.link; // 방문했다면 다음 링크로 이어감
		}
		
		Node newNode = new Node(); // r의 맨 앞에 정점 v 삽입
		newNode.vertex = v;
		newNode.link = r;
		r = newNode;
	} 
